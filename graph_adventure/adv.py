...
roomGraph={0: [(3, 5), {'n': 1, 'e': 2}], 1: [(3, 6), {'s': 0}], 2: [(4, 5), {'w': 0, 'n': 4, 'e': 5}], 3:[(5,6), {'s': 5}], 4:[(4,6), {'s': 2}], 5: [(5,5), {'w': 2, 'n': 3}]}
...
while len(s) > 0:
    ...
    if len(exits) == 1:
        if len(visited_rooms) == len(roomGraph):
            break
        currentRoom = v.id # 18
        while currentRoom not in intersection_rooms: # 18
            currentRoom = parent[currentRoom][0] # 12
            path.append(currentRoom) # add 12
    else:
        for direction in exits:
            room = v.getRoomInDirection(direction)
